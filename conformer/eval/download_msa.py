# Taken from ColabFold: https://github.com/sokrypton/ColabFold/blob/main/colabfold/colabfold.py
# Some modifications to integrate with Conformer
...
def download_msa(eval_inputs):
    for tag in eval_inputs.all_tags:
        assert eval_inputs.config.use_env
        temp_dir = os.path.join(eval_inputs.output_dir, f"{tag}_temp")
        os.makedirs(temp_dir, exist_ok=True)
        protein_sequence = eval_inputs.sequence_dict[tag]
        a3m_lines = run_mmseqs2(
            [protein_sequence],
            temp_dir,
            eval_inputs.config.use_env,  # use_env
            use_pairing=False,
            host_url="https://api.colabfold.com",
        )
        output_dir = os.path.join(eval_inputs.alignment_dir, tag)
