# Taken from https://github.com/HWaymentSteele/AF_Cluster
import argparse
import pandas as pd
...
from conformer.utils.af_cluster_utils import *
...
def cluster_msa(args, loaded_msa=None):
    if args.run_PCA:
    ...
    for clust in clusters:
        ...
        output_path = os.path.join(
            args.o, args.keyword + "_" + "%03d" % clust
        )  # args.o+'/'+args.keyword+'_'+"%03d" % clust+'.a3m'
        os.makedirs(os.path.join(output_path, args.keyword), exist_ok=True)
        all_output_paths.append(output_path)
        write_fasta(
            tmp.SequenceName.tolist(),
            tmp.sequence.tolist(),
            outfile=os.path.join(
                output_path, args.keyword, args.keyword + "_" + "%03d" % clust + ".a3m"
            ),
        )
    ...
    for i in range(args.n_controls):
        tmp = df.sample(n=10)
        tmp = pd.concat([query_, tmp], axis=0)
        output_path = os.path.join(args.o, args.keyword + "_U10-" + "%03d" % i)
        os.makedirs(os.path.join(output_path, args.keyword), exist_ok=True)
        uniform_output_paths_10.append(output_path)
        write_fasta(
            tmp.SequenceName.tolist(),
            tmp.sequence.tolist(),
            outfile=os.path.join(
                output_path, args.keyword, args.keyword + "_U10-" + "%03d" % i + ".a3m"
            ),
        )
    ...
    if len(df) > 100:
        ...
        for i in range(args.n_controls):
            tmp = df.sample(n=100)
            tmp = pd.concat([query_, tmp], axis=0)
            output_path = os.path.join(args.o, args.keyword + "_U100-" + "%03d" % i)
            os.makedirs(os.path.join(output_path, args.keyword), exist_ok=True)
            uniform_output_paths_100.append(output_path)
            write_fasta(
                tmp.SequenceName.tolist(),
                tmp.sequence.tolist(),
                outfile=os.path.join(
    ...
    f.close()
    return all_output_paths, uniform_output_paths_10, uniform_output_paths_100
